...
public class EtudiantServiceImpl implements IEtudiantService {
    ...
    public EtudiantDto updateEtudiantWithFiles(Long id, EtudiantDto etudiantDto, MultipartFile photoFile, MultipartFile cvFile) throws IOException {
        ...
        // Mettre à jour les champs si les valeurs ne sont pas nulles ou vides
        if (etudiantDto.getNom() != null && !etudiantDto.getNom().trim().isEmpty()) {
            existingEtudiant.setNom(etudiantDto.getNom());
        }
        if (etudiantDto.getPrenom() != null && !etudiantDto.getPrenom().trim().isEmpty()) {
            existingEtudiant.setPrenom(etudiantDto.getPrenom());
        }
        if (etudiantDto.getMatricule() != null && !etudiantDto.getMatricule().trim().isEmpty()) {
            existingEtudiant.setMatricule(etudiantDto.getMatricule());
        }
        if (etudiantDto.getFiliere() != null && !etudiantDto.getFiliere().trim().isEmpty()) {
